...
export default function GaransiPage() {
  ...
  return (
    <>
      ...
      {/* Konten utama */}
      <main className="min-h-screen flex flex-col justify-center items-center px-4 pt-32 pb-20 theme-shell">
        <div className="w-full max-w-lg p-6 sm:p-8 premium-card rounded-3xl">
          <motion.h1
          ...
          <form onSubmit={handleCheckWarranty} className="space-y-5">
            <div className="space-y-2">
              ...
              <Input
                ...
                className="h-14 text-base premium-input focus:border-violet-400 focus:ring-violet-400/30"
              />
            </div>

            <Button
              type="submit"
              className="w-full premium-button h-14 text-lg font-semibold transition-all duration-300 ease-in-out transform hover:scale-[1.01]"
              disabled={isLoading}
            >
              {isLoading ? (
                <>
          ...
          <Dialog open={showDialog} onOpenChange={setShowDialog}>
            <DialogContent className="sm:max-w-md premium-card border-white/10 text-white">
              ...
              {garansiData && dialogType === "valid" && (
                <div className="bg-white/[0.04] p-4 rounded-xl border border-white/10 space-y-3">
                  <div className="grid grid-cols-2 gap-2 text-sm">
                    ...
                    <div className="font-medium text-cyan-300">{formatRupiah(garansiData.amount)}</div>
              ...
              <DialogFooter className="flex flex-col sm:flex-row sm:justify-between gap-2">
                ...
                {dialogType === "expired" && (
                  <Button onClick={() => router.push("/")} className="w-full sm:w-auto premium-button">
                    Beli Lagi
                  </Button>
                )}
