    ...
    <AnimatePresence>
      {isLoading && (
        <motion.div
          ...
          className="fixed inset-0 z-50 flex items-center justify-center theme-shell"
        >
          <div className="flex flex-col items-center">
            ...
            >
              <div className="relative">
                <ShoppingCart className="w-20 h-20 text-violet-400" />
                <motion.div
                  initial={{ width: 0 }}
                  animate={{ width: "100%" }}
                  transition={{ duration: 1.5, ease: "easeInOut", repeat: Number.POSITIVE_INFINITY }}
                  className="absolute bottom-0 left-0 h-1 bg-gradient-to-r from-violet-500 via-fuchsia-400 to-cyan-400 rounded-full"
                />
              </div>
            </motion.div>
            ...
            >
              <span className="bg-gradient-to-r from-violet-500 via-fuchsia-400 to-cyan-400 bg-clip-text text-transparent">{appConfig.nameHost}</span>
            </motion.h1>
